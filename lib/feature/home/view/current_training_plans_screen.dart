...
import '../controller/current_training_plans_controller.dart';
...
class _CurrentTrainingPlansScreenState
  ...
  final CurrentTrainingPlansController _controller =
      CurrentTrainingPlansController();
  ...
  Future<void> _loadOwnedPlans() async {
    ...
    try {
      final ordered = await _controller.loadOwnedDestinations(
        preferredDestination: widget.preferredDestination,
      );
      ...
      final message = _controller.readErrorMessage(
        error,
        fallback: 'Failed to load your current plans',
      );
      if (!mounted) return;
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          TextButton(onPressed: onRetry, child: const TranslatedText('')),
        ],
      ),
    );
  }
